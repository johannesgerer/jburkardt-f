...
!*****************************************************************************80
!
!! I4VEC2_COMPARE compares pairs of integers stored in two I4VEC's.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
        ...
        if ( position_num > position_max ) then
          ...
          write ( *, '(a,i6)' ) &
            '  The provided space is POSITION_MAX = ', position_max
          stop
        end if
...
subroutine ps_gg_bpq ( m, m1, m2, n, n1, n2, lds, t, npath, pathi, pathj )

!*****************************************************************************80
!
!! PS_GG_BPQ determines a global gap backward alignment path in quadratic space.
!
!  Discussion:
!
...
!
    ...
    if ( j < n2 .and. i_new == 1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'PS_GG_BPQ: Unexpected situation!'
      write ( *, '(a,2i6)' ) '  I, J = ', i, j
...
!
!    Input, integer ( kind = 4 ) M1, M2, the first and last rows of the
!    table to compute.
!
...
subroutine ps_gg_bsq ( b, m, m1, m2, n, n1, n2, ps_score, go, ge, &
  base, lds, s, e, f, t )

!*****************************************************************************80
!
!! PS_GG_BSQ determines a global gap backward alignment score in quadratic space.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
  if ( n2-1 >= n1 ) then
    ...
    f(m2,n2-1) = f(m2,n2) + 2.0 * go(m2) + ge(m2)
...
!
  do i = m2-1, m1, -1
    ...
    if ( i == m2-1 ) then
      e(i,n2) = base + ge(i) + 2.0 * go(i) -       go(i+1)
      f(i,n2) = base + ge(i) + 2.0 * go(i) - 2.0 * go(i+1)
      s(i,n2) = base + ge(i)
      t(i,n2) = DELETE
    else
      e(i,n2) = e(i+1,n2) + ge(i) + 2.0 * go(i) - 2.0 * go(i+1)
      f(i,n2) = f(i+1,n2) + ge(i) + 2.0 * go(i) - 2.0 * go(i+1)
...
subroutine ps_gg_fpq ( m, m1, m2, n, n1, n2, lds, t, npath, pathi, pathj )

!*****************************************************************************80
!
!! PS_GG_FPQ determines a global gap forward alignment path in quadratic space.
!
!  Discussion:
!
!    An effort has been made to handle the ambiguous case, where
...
!
    ...
    if ( j > n1 .and. i_new == 1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'PS_GG_FPQ: Unexpected situation!'
      write ( *, '(a,2i6)' ) '  I, J = ', i, j
      write ( *, '(a,i6)' ) '  T(I,J) = ', t(i,j)
      stop
...
subroutine ps_gg_fsq ( b, m, m1, m2, n, n1, n2, ps_score, go, ge, &
  base, lds, s, e, f, t )

!*****************************************************************************80
!
!! PS_GG_FSQ determines a global gap forward alignment score in quadratic space.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!  Upper Left corner.
!
!  (I could be persuaded that these zero values should really be BASE)
!
  e(m1,n1) = 0.0
  f(m1,n1) = 0.0
  s(m1,n1) = 0.0
...
!  Upper Right row.
!
!  (I can't recall two days in a row why there's a 2.0 * go(m1)...
!
  if ( n1+1 <= n2 ) then
    e(m1,n1+1) = e(m1,n1) + go(m1) + ge(m1)
    f(m1,n1+1) = f(m1,n1) + 2.0 * go(m1) + ge(m1)
...
subroutine ps_gg_match_print ( b, m, n, npath, pathi, pathj, ps_score, &
  go, ge )

!*****************************************************************************80
!
!! PS_GG_MATCH_PRINT prints a global gap alignment.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
!    Input, integer ( kind = 4 ) PATHI(M+N+1), PATHJ(M+N+1), contains, in the
...
subroutine ps_gg_rpl ( b, m, m1, m2, n, n1, n2, ps_score, go, ge, &
  npath, pathi, pathj )

!*****************************************************************************80
!
!! PS_GG_RPL determines a global gap recursive alignment path in linear space.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!    Input, integer ( kind = 4 ) M, the number of entries in the profile.
!
!    Input, integer ( kind = 4 ) M1, M2, the minimum and maximum entries of
!    A to align.  For a full alignment, use M1 = 0, M2 = M.
!
!    Input, integer ( kind = 4 ) N, the number of entries in the sequence.
!
!    Input, integer ( kind = 4 ) N1, N2, the minimum and maximum entries of B
...
!
  ...
  call ps_gg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, t2, &
...
!
    call ps_gg_rpl_pop ( i1sub, j1sub, j1type, t1, i3sub, j3sub, j3type, t3, &
      stack1, stack2, stack_num, stack_max )
!
...
!
      ...
      if ( j2type == MATCH ) then
        ...
        call ps_gg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
          t2, stack1, stack2, stack_num, stack_max )

        call ps_gg_rpl_push ( i2sub, j2sub, j2type, t2, i3sub, j3sub, j3type, &
        ...
        if ( ( i1sub < i2sub-1 .or. j1sub < j2sub ) .and. &
          ...
          call ps_gg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub-1, j2sub, &
            j2type, t2, stack1, stack2, stack_num, stack_max )

          call ps_gg_rpl_push ( i2sub+1, j2sub, j2type, t2, i3sub, j3sub, &
...
subroutine ps_gg_rpl_pop ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
  t2, stack1, stack2, stack_num, stack_max )

!*****************************************************************************80
!
!! PS_GG_RPL_POP pops the data describing a subproblem off of the stack.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!  Parameters:
!
!    Output, integer ( kind = 4 ) I1SUB, J1SUB, J1TYPE, real T1, the row and
!    column of the first cell, the type of the match there, and the appropriate
!    base.
!
!    Output, integer ( kind = 4 ) I2SUB, J2SUB, J2TYPE, real T2, the row and
!    column of the second cell, the type of the match there, and the appropriate
!    base.
!
...
!
  ...
  if ( stack_num < 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'PS_GG_RPL_POP - Fatal error!'
...
subroutine ps_gg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
  t2, stack1, stack2, stack_num, stack_max )

!*****************************************************************************80
!
!! PS_GG_RPL_PUSH pushes the data describing a subproblem onto the stack.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) I1SUB, J1SUB, J1TYPE, real T1, the row and
!    column of the first cell, the type of the match there, and the appropriate
!    base.
!
...
!
  if ( stack_num >= stack_max ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'PS_GG_RPL_PUSH - Fatal error!'
...
!*****************************************************************************80
!
!! S_TO_CHVEC converts a string to a CHVEC.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.

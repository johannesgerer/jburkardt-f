...
!
!  Next code depends on element order.
!
...
!
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TET_MESH_RCM - Fatal error!'
    write ( *, '(a)' ) '  This order 10 code is not ready.'
    stop
...
!
!    Input, integer ( kind = 4 ) ADJ_ROW(NODE_NUM+1).  Information about row I is stored
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) PERM(NODE_NUM), integer PERM_INV(NODE_NUM), the permutation
...
!    Input, integer ( kind = 4 ) ADJ_NUM, the number of adjacency entries.
!
!    Input, integer ( kind = 4 ) ADJ_ROW(NODE_NUM+1), organizes the adjacency entries
!    into rows.  The entries for row I are in entries ADJ_ROW(I)
!    through ADJ_ROW(I+1)-1.
!
!    Input, integer ( kind = 4 ) ADJ(ADJ_NUM), the adjacency structure, which contains,
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) MASK(NODE_NUM), is nonzero for those nodes which are
!    to be considered.
!
!    Output, integer ( kind = 4 ) DEG(NODE_NUM), contains, for each  node in the connected
!    component, its degree.
!
!    Output, integer ( kind = 4 ) ICCSIZE, the number of nodes in the connected component.
!
!    Output, integer ( kind = 4 ) LS(NODE_NUM), stores in entries 1 through ICCSIZE the nodes
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input/output, integer ( kind = 4 ) MASK(NODE_NUM).  On input, only nodes with nonzero
...
!
!    Output, integer ( kind = 4 ) LEVEL_ROW(NODE_NUM+1), LEVEL(NODE_NUM), the rooted 
!    level structure.
!
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input/output, integer ( kind = 4 ) MASK(NODE_NUM), a mask for the nodes.  Only 
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) MASK(NODE_NUM), specifies a section subgraph.  Nodes 
!    for which MASK is zero are ignored by FNROOT.
!
!    Output, integer ( kind = 4 ) LEVEL_NUM, is the number of levels in the level structure
!    rooted at the node ROOT.
!
!    Output, integer ( kind = 4 ) LEVEL_ROW(NODE_NUM+1), integer LEVEL(NODE_NUM), the 
...
!    Input, integer ( kind = 4 ) TETRA_NODE(4,TETRA_NUM), the indices of the nodes.
!
!    Input, integer ( kind = 4 ) ADJ_NUM, the total number of adjacency relationships,
!

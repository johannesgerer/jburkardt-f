...
!
  ...
  else if (lensav < 2*n + int(log( real ( n, kind = 4 ) )/log( 2.0E+00 )) + 4) then
    ier = 2
...
!
  ...
  if ( ldim < l ) then
    ...
    return
  else if (lensav < 2*l + int(log( real ( l, kind = 4 ))/log( 2.0E+00 )) + &
                    2*m + int(log( real ( m, kind = 4 ))/log( 2.0E+00 )) +8) then
    ier = 2
...
!
  iw = 2*l+int(log( real ( l, kind = 4 ) )/log( 2.0E+00 )) + 3

  call cfftmb(l, 1, m, ldim, c, (l-1) + ldim*(m-1) +1, &
    wsave(iw), 2*m + int(log( real ( m, kind = 4 ))/log( 2.0E+00 )) + 4, &
    work, 2*l*m, ier1)
...
!
  ...
  call cfftmb (m, ldim, l, 1, c, (m-1)*ldim + l, &
    wsave(iw), 2*l + int(log( real ( l, kind = 4 ) )/log( 2.0E+00 )) + 4, &
    work, 2*m*l, ier1)
...
!
  ...
  if ( ldim < l ) then
    ...
    2*l + int(log( real ( l, kind = 4 ))/log( 2.0E+00 )) + &
    2*m + int(log( real ( m, kind = 4 ))/log( 2.0E+00 )) +8) then
    ier = 2
...
!
  ...
  if ( lensav < 2 * l + int ( log ( real ( l, kind = 4 ) ) &
    / log ( 2.0E+00 ) ) + 2 * m + int ( log ( real ( m, kind = 4 ) ) &
    / log ( 2.0E+00 ) ) + 8 ) then
    ier = 2
  ...
  call cfftmi ( l, wsave(1), 2 * l + int ( log ( real ( l, kind = 4 ) ) &
    / log ( 2.0E+00 ) ) + 4, ier1 )
  ...
  call cfftmi ( m, &
    wsave(2*l+int(log( real ( l, kind = 4 ) )/log( 2.0E+00 )) + 3), &
    2*m + int(log( real ( m, kind = 4 ) )/log( 2.0E+00 )) + 4, ier1)
...
!
  ...
  if (lenx < inc*(n-1) + 1) then
    ...
    2*n + int(log( real ( n, kind = 4 ) )/log( 2.0E+00 )) +4) then
    ier = 2
...
!
  ...
  lnsv = n + int(log( real ( n, kind = 4 ) )/log( 2.0E+00 )) +4
...
!
  ...
  lnsv = nm1 + int(log( real ( nm1, kind = 4 ) )/log( 2.0E+00 )) +4
...
!
  ...
  124 lenx = inc*(nm1-1)  + 1
   ...
   30 fnm1s4 = real ( nm1, kind = 4 ) / 4.0E+00
...
!
  ...
  ti12=sin( 2.0E+00 *arg)
      ...
      do 1001 m1=1,m1d,im1
         m2 = m2+im2
         ch(m2,1,k,1) = cc(m1,1,1,k)+ 2.0E+00 *cc(m1,ido,2,k)+ 2.0E+00 *cc(m1,ido,4,k)
...
!
  ...
  lj = (lot-1)*jump+1
      ...
      if (n-2) 200,102,103
 102  srt3s2 = sqrt( 3.0E+00 )/ 2.0E+00 
  ...
  124 do m=1,lot
      ...
      lnsv = np1 + int(log( real ( np1, kind = 4 ))/log( 2.0E+00 )) + 4
      lnwk = lot*np1
...
!
  ...
  lj = (lot-1)*jump+1
      ...
      if (n-2) 101,102,103
 102  ssqrt3 = 1.0E+00 / sqrt ( 3.0E+00 )
  ...
  127 continue 
      ...
      xh(m,np1) = xh(m,np1)+xh(m,np1)
   20 continue
   30 sfnp1 = 1.0E+00 / real ( np1, kind = 4 )
      m1 = 0
      do 125 m=1,lj,jump
         m1 = m1+1
...
!  verify lensav
!
  lwsav =   l+int(log( real ( l, kind = 4 ) )/log( 2.0E+00 ))+4
  mwsav =   2*m+int(log( real ( m, kind = 4 ) )/log( 2.0E+00 ))+4
  mmsav =   m+int(log( real ( m, kind = 4 ) )/log( 2.0E+00 ))+4
  modl = mod(l,2)
  modm = mod(m,2)
...
!
      ...
      call rfftmb(m,ldim,l,1,r,m*ldim,wsave(1), &
           l+int(log( real ( l, kind = 4 ) )/log( 2.0E+00 ))+4,work,lenwrk,ier1)
...
!
  ...
  if (lenx < inc*(n-1) + 1) then
    ...
    return
  else if ( lensav < n / 2 + n + int ( log ( real ( n, kind = 4 ) ) &
    / log ( 2.0E+00 ) ) + 4 ) then
    ier = 2
...
!
  ...
  if (lenx < inc*(n-1) + 1) then
    ...
    return
  else if (lensav < n/2 + n + int(log( real ( n, kind = 4 ) )/log( 2.0E+00 )) +4) then
    ier = 2
...
!
  ...
  if (lensav < n/2 + n + int(log( real ( n, kind = 4 ) )/log( 2.0E+00 )) +4) then
    ier = 2
...
!
 ...
 30   fnp1s4 = real ( np1, kind = 4 ) / 4.0E+00
         x(1,1) = fnp1s4*xh(1)
         dsum = x(1,1)
      do i=3,n,2
            x(1,i-1) = fnp1s4*xh(i)
...
!
  ...
  if ( lensav < n / 2 + n + int ( log ( real ( n, kind = 4 ) ) &
    / log ( 2.0E+00 ) ) + 4 ) then
    ier = 2

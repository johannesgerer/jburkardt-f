#!/bin/bash
#
gfortran -c -g test_int_hermite_prb.f90 >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling test_int_hermite_prb.f90"
  exit
fi
rm compiler.txt
#
gfortran test_int_hermite_prb.o -L$HOME/lib/$ARCH -ltest_int_hermite
if [ $? -ne 0 ]; then

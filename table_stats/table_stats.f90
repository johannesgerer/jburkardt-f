...
!*****************************************************************************80
!
!! MAIN is the main program for TABLE_STATS.
!
!  Discussion:
!
!    TABLE_STATS reads the data from a table file and prints
!    statistics.
!
!  Licensing:
!
...
!
!  For each component, compute average, max, min and range.
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '       I       Minimum    Average    Maximum    Range'
  write ( *, '(a)' ) ' '

  do i = 1, m
    xmax = maxval ( table(i,1:n) )
    xmin = minval ( table(i,1:n) )
    xave = sum ( table(i,1:n) ) / real ( n, kind = 8 )
    xrange = xmax - xmin
    write ( *, '(2x,i8,4g14.6)' ) i, xmin, xave, xmax, xrange
  end do
!
!  Free memory.
!

...
!
  ...
  interface
    function quad_compute ( n, comm ) bind ( c )
      use iso_c_binding
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  This means that the C function must receive the value'
  write ( *, '(a)' ) '  of MPI_COMM_WORLD from the FORTRAN program.'
!
!  Initialize MPI.
!
  call MPI_Init ( ierr )

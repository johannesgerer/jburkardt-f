#!/bin/bash
#
gfortran -c -g sparse_grid_mixed_weight_prb.f90 >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling sparse_grid_mixed_weight_prb.f90"
  exit
fi
rm compiler.txt
#
gfortran sparse_grid_mixed_weight_prb.o -L$HOME/lib/$ARCH -lsparse_grid_mixed -lsandia_rules
if [ $? -ne 0 ]; then

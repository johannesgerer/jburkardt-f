...
!
  if ( 2 <= arg_num ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_HISTOGRAM:'
    write ( *, '(a)' ) '  Please enter N = 1, 2, 3, ..., the order of refinement.'
...
!
  do d = 1, data_num
    if ( data_xy(1,d) < 0.0D+00 .or. &
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'TRIANGLE_HISTOGRAM - Fatal error!'
      write ( *, '(a,i8,a)' ) '  Point ', d, ' does not lie inside the triangle.'
...
!
  do d = 1, data_num
    ...
    i = int (             data_xy(1,d)                  * real ( n, kind = 8 ) ) + 1
    j = int (                            data_xy(2,d)   * real ( n, kind = 8 ) ) + 1
    k = int ( ( 1.0D+00 - data_xy(1,d) - data_xy(2,d) ) * real ( n, kind = 8 ) ) + 1
...
!
  histo_ave = real ( sum ( histo(1:sub_num) ), kind = 8 ) / real ( sub_num, kind = 8 )
...
!
!    Output, real ( kind = 8 ) STD, the standard deviation of the vector entries.
!
  implicit none

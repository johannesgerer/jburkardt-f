...
!
!    Input, integer ( kind = 4 ) ORDER_NUM, the order of the triangles.
!
!    Input, integer ( kind = 4 ) TRIANGLE_NUM, the number of triangles.
!
!    Input, real ( kind = 8 ) NODE_XYZ(DIM_NUM,NODE_NUM), the node coordinates.
!
!    Input, integer ( kind = 4 ) TRIANGLE_NODE(ORDER_NUM,TRIANGLE_NUM),
...
!
  ...
  integer   ( kind = 4 ) triangle_node(order_num,triangle_num)
...
!    Input, character ( len = * ) NODE_FILE_NAME, the name of the node file.
!
!    Input, character ( len = * ) TRIANGLE_FILE_NAME, the name of the triangle file.
!
!    Input, integer ( kind = 4 ) DIM_NUM, the spatial dimension.
!
!    Input, integer ( kind = 4 ) NODE_NUM, the number of points.
...
!    Input, integer ( kind = 4 ) TRIANGLE_NUM, the number of triangles.
!
!    Output, real ( kind = 8 ) NODE_XYZ(DIM_NUM,NODE_NUM), the node coordinates.
!
!    Output, integer ( kind = 4 ) TRIANGLE_NODE(ORDER_NUM,TRIANGLE_NUM),
!    the nodes that form the triangles.
!
  implicit none
...
!
!    Output, integer ( kind = 4 ) ORDER_NUM, the order of the triangles.
!
...
subroutine tri_surface_size_print ( node_file_name, triangle_file_name, dim_num, &
  node_num, order_num, triangle_num )

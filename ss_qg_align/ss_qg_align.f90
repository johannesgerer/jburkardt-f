...
!*****************************************************************************80
!
!! I4VEC2_PRINT prints an I4VEC2 with an optional title.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!*****************************************************************************80
!
!! R4VEC2_SUM_IMAX returns the index of the maximum sum of an R4VEC2.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
!      -1 if the characters are not identical,
!       2 if the characters are identical.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
subroutine ss_qg_boq ( m, m1, m2, n, n1, n2, lds, s, i1, j1 )

!*****************************************************************************80
!
!! SS_QG_BOQ: backward endpoint of a quasiglobal optimal local alignment.
!
!  Discussion:
!
...
!    Input, integer M, the number of entries in sequence A.
!
!    Input, integer M1, M2, the minimum and maximum rows to consider.
!
!    Input, integer N, the number of entries in sequence B.
!
!    Input, integer N1, N2, the minimum and maximum columns to consider.
!
...
subroutine ss_qg_bpq ( i1, j1, m, m1, m2, n, n1, n2, lds, t, npath, pathi, &
  pathj )

!*****************************************************************************80
!
!! SS_QG_BPQ: quasiglobal gap backward alignment path in quadratic space.
!
!  Discussion:
!
...
!
    if ( t_new == 1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'SS_QG_BPQ: Unexpected situation!'
      write ( *, '(a,i6)' ) '  I = ', i
      write ( *, '(a,i6)' ) '  J = ', j
      write ( *, '(a,g14.6)' ) '  T(I,J) = ', t(i,j)
      stop
...
subroutine ss_qg_bsl ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, base, s, e, f, t, i1, j1, s_max )

!*****************************************************************************80
!
!! SS_QG_BSL: quasiglobal gap backward alignment score in linear space.
!
!  Discussion:
!
!    The routine is called "linear" because it uses one N vector,
...
!    Output, integer T(0:N), the backward pointer vector.
!
!    Output, integer I1, J1, the location of an optimal quasiglobal score.
!    I1 will equal M1, or J1 will equal N1.
!
...
subroutine ss_qg_bsq ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, base, lds, s, e, f, t )

!*****************************************************************************80
!
!! SS_QG_BSQ: quasiglobal gap backward alignment score in quadratic space.
!
!  Discussion:
!
...
subroutine ss_qg_foq ( m, m1, m2, n, n1, n2, lds, s, i2, j2 )

!*****************************************************************************80
!
!! SS_QG_FOQ: forward endpoint of a quasiglobal optimal local alignment.
!
!  Discussion:
!
...
!    Entries in the M1:M2 by N1:N2 block have been computed.
!
!    Output, integer I2, J2, the beginning of an optimal quasiglobal alignment.
!    I2 is equal to M2, or J2 is equal to N2.
!
  implicit none
...
subroutine ss_qg_fpq ( i2, j2, m, m1, m2, n, n1, n2, lds, t, npath, pathi, &
  pathj )

!*****************************************************************************80
!
!! SS_QG_FPQ: quasiglobal gap forward alignment path in quadratic space.
!
!  Discussion:
!
!    The routine must be given a starting point.
...
!
    if ( t_new == 1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'SS_QG_FPQ: Unexpected situation!'
      write ( *, '(a,i6)' ) '  I = ', i
      write ( *, '(a,i6)' ) '  J = ', j
      write ( *, '(a,g14.6)' ) '  T(I,J) = ', t(i,j)
      stop
...
subroutine ss_qg_fsl ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, base, s, e, f, t, i2, j2, s_max )

!*****************************************************************************80
!
!! SS_QG_FSL: quasiglobal gap forward alignment score in linear space.
!
!  Discussion:
!
!    The routine is called "linear" because it uses one N vector,
...
!    Output, integer T(0:N), the forward pointer vector.
!
!    Output, integer I2, J2, the location of an optimal quasiglobal score.
!    I2 will equal M2, or J2 will equal N2.
!
!    Output, real S_MAX, the optimal quasiglobal score.
!
  implicit none
...
subroutine ss_qg_fsq ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, base, lds, s, e, f, t )

!*****************************************************************************80
!
!! SS_QG_FSQ: quasiglobal gap forward alignment score in quadratic space.
!
!  Discussion:
!
!    The routine can compute the full score table, or a sub-block.
...
subroutine ss_qg_match_print ( a, b, m, n, npath, pathi, pathj, ss_score, &
  gap_open, gap_extend )
...
subroutine ss_qg_match_score ( a, b, m, n, npath, pathi, pathj, ss_score, &
...
subroutine ss_qg_rpl ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, npath, pathi, pathj )

!*****************************************************************************80
!
!! SS_QG_RPL: quasiglobal gap recursive alignment path in linear space.
!
!  Discussion:
!
...
!
  ...
  call ss_qg_bsl ( a, b, m, m1, m2, n, n1, n2, ss_score, &
    gap_open, gap_extend, base, sb, eb, fb, tb, i1, j1, s_max )

  base = 0.0

  call ss_qg_fsl ( a, b, m, m1, m2, n, n1, n2, ss_score, &
    gap_open, gap_extend, base, sf, ef, ff, tf, i2, j2, s_max )
...
!
  ...
  call ss_qg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, t2, &
...
!
    call ss_qg_rpl_pop ( i1sub, j1sub, j1type, t1, i3sub, j3sub, j3type, t3, &
      stack1, stack2, stack_num, stack_max )
!
...
!
      ...
      if ( j2type == MATCH ) then
        ...
        call ss_qg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
          t2, stack1, stack2, stack_num, stack_max )

        call ss_qg_rpl_push ( i2sub, j2sub, j2type, t2, i3sub, j3sub, j3type, &
        ...
        if ( ( i1sub < i2sub-1 .or. j1sub < j2sub ) .and. &
          ...
          call ss_qg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub-1, j2sub, &
            j2type, t2, stack1, stack2, stack_num, stack_max )

          call ss_qg_rpl_push ( i2sub+1, j2sub, j2type, t2, i3sub, j3sub, &
...
subroutine ss_qg_rpl_pop ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
  t2, stack1, stack2, stack_num, stack_max )
...
!
  ...
  if ( stack_num < 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'SS_QG_RPL_POP - Fatal error!'
...
subroutine ss_qg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
  t2, stack1, stack2, stack_num, stack_max )
...
!
  if ( stack_num >= stack_max ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'SS_QG_RPL_PUSH - Fatal error!'

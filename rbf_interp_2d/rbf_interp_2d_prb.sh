#!/bin/bash
#
gfortran -c -g rbf_interp_2d_prb.f90 >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling rbf_interp_2d_prb.f90"
  exit
fi
rm compiler.txt
#
gfortran rbf_interp_2d_prb.o -L$HOME/lib/$ARCH -lrbf_interp_2d -ltest_interp_2d -lr8lib
if [ $? -ne 0 ]; then

...
!
!    Input, complex A1(M*M,L), the first row of blocks of the matrix.
!    Each block is represented by columns.
!
!    Input, complex A2(M*M,L-1), the first column of blocks of the matrix,
...
!  Parameters:
!
!    Input/output, complex A(M*M*L,K).
...
!  Parameters:
!
!    Input/output, complex A(M*M*(2*L - 1),K).
...
!    On output, the solution vector.
!
!    Workspace, complex R(max(M*M*(2*L + 3) + M,2*K)).
!
!    Input, integer ( kind = 4 ) M, the order of the inner blocks of the matrix A.
!
!    Input, integer ( kind = 4 ) L, the number of inner blocks in a row or column
...
!  Parameters:
!
!    Input, complex A(LDA,2*L-1), an M*M by 2*L-2 array, containing
...
!
!    Workspace, complex R(M*M*(2*L+3)+M).
!
...
!  Parameters:
!
!    Input, complex A1(M*M,L), the first row of blocks of the TG matrix.
!    Each block is represented by columns.
!
!    Input, complex A2(M*M,L-1), the first column of blocks of the TG
...
!    entries of X.
!
!    Input/output, real ( kind = 4 ) Y(*), the vector to which a multiple of X is to
!    be added.
!
!    Input, integer ( kind = 4 ) INCY, the increment between successive
!    entries of Y.
...
!    Input, integer ( kind = 4 ) L, the number of blocks in a row or column of A.
!
!    Input, real ( kind = 4 ) A1(M,M,L), the M*M by L matrix containing the first row of
!    blocks of the matrix.  There are L blocks, and each is of order M*M.
!
!    Input, real ( kind = 4 ) A2(M,M,L-1), the M*M by L-1 matrix containing the first
...
!
!    Output, real ( kind = 4 ) X(M*L), the solution vector.  X and B may share storage.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = 4 ) A(M), the first column of the column-circulant matrix.
!
...
!
!    Output, real ( kind = 4 ) Q(LDQ,L), the M by L matrix Q of the factorization.
!    The columns of Q are orthonormal.
!
!    Output, real ( kind = 4 ) S(LDS,L), the L by L inverse of the R matrix of the
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 4 ) A(2*N-1), the entries of the first row of the Toeplitz
!    matrix, followed by the entries of the first column, beginning
!    with the second row.
!
...
!    N must be positive.
!
!    Output, real ( kind = 4 ) A(2*N-1), the randomized matrix, with entries between
!    0 and 1.
!
  implicit none
...
!    Input, integer ( kind = 4 ) N, the order of the matrix T.
!
!    Output, real ( kind = 4 ) DET(2), the determinant of the matrix, if requested.

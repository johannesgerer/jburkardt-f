...
!
  ...
  temp = temp + dble ( values(2) - 1 ) / 11.0E+00
  temp = temp + dble ( values(3) - 1 ) / 30.0E+00
  temp = temp + dble ( values(5) ) / 23.0E+00
  temp = temp + dble ( values(6) ) / 59.0E+00
  temp = temp + dble ( values(7) ) / 59.0E+00
  temp = temp + dble ( values(8) ) / 999.0E+00
  temp = temp / 6.0E+00

  if ( temp <= 0.0E+00 ) then
    temp = 1.0E+00 / 3.0E+00
  else if ( temp >= 1.0E+00 ) then
    temp = 2.0E+00 / 3.0E+00
  end if
...
!*****************************************************************************80
!
!! R4MAT_IMAX returns the location of the maximum of an R4MAT.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!
  ...
  if ( n2-1 >= n1 ) then
    ...
    f(n2-1) = f(n2) + 2.0E+00 * gap_open + gap_extend
...
!
  ...
  if ( n1+1 <= n2 ) then
    ...
    f(n1+1) = f(n1) + 2.0E+00 * gap_open + gap_extend
...
subroutine ss_lg_bpq ( i1, j1, m, m1, m2, n, n1, n2, lds, t, npath, pathi, &
  pathj )
...
!
    if ( t_new == 1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'SS_LG_BPQ: Unexpected situation!'
      write ( *, '(a,i6)' ) '  I = ', i
      write ( *, '(a,i6)' ) '  J = ', j
...
subroutine ss_lg_bsl ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, base, s, e, f, t )
...
!    Input, real GAP_OPEN, GAP_EXTEND, the penalties for opening and
!    extending a gap.  A gap of length 7, for example, will result in a
!    penalty of GAP_OPEN + 7 * GAP_EXTEND.  We assume GAP_OPEN and GAP_EXTEND
!    are negative.
!
!    Input, real BASE, an initial quantity to be added to certain penalties.
!
...
subroutine ss_lg_bsq ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
...
!
  if ( n2-1 >= n1 ) then
    ...
    f(m2,n2-1) = f(m2,n2) + 2.0E+00 * gap_open + gap_extend
...
subroutine ss_lg_corners ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
...
!    are negative.
!
!    Output, integer I1, J1, I2, J2, the "corners" of an optimal local 
!    alignment.
!
  implicit none
  ...
  do mhi = m1, m2
    ...
    call ss_lg_fsl ( a, b, m, m1, mhi, n, n1, n2, ss_score, gap_open, &
      gap_extend, base, s, e, f, t )
  ...
  do mlo = i2, m1, -1
    ...
    call ss_lg_bsl ( a, b, m, mlo, i2, n, n1, j2, ss_score, gap_open, &
      gap_extend, base, s, e, f, t )
...
subroutine ss_lg_fpq ( i1, j1, m, m1, m2, n, n1, n2, lds, t, npath, pathi, &
  pathj )
...
!
    if ( t_new == 1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'SS_LG_FPQ: Unexpected situation!'
      write ( *, '(a,i6)' ) '  I = ', i
      write ( *, '(a,i6)' ) '  J = ', j
      write ( *, '(a,i6)' ) '  T(I,J) = ', t(i,j)
      stop
...
subroutine ss_lg_fsl ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, base, s, e, f, t )
...
!    Michael Waterman,
!    Introduction to Computational Biology,
!    Chapman and Hall, 1995, pages 202-206.
!
!  Parameters:
!
!    Input, character A(M), B(N), two sequences to be aligned.
...
subroutine ss_lg_fsq ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
  gap_extend, base, lds, s, e, f, t )
...
!
  if ( n1+1 <= n2 ) then
    ...
    f(m1,n1+1) = f(m1,n1) + 2.0E+00 * gap_open + gap_extend
...
subroutine ss_lg_match_print ( a, b, m, n, npath, pathi, pathj, ss_score, &
  gap_open, gap_extend )
...
subroutine ss_lg_match_score ( a, b, m, n, npath, pathi, pathj, ss_score, &
...
subroutine ss_lg_rpl ( a, b, m, m1, m2, n, n1, n2, ss_score, gap_open, &
...
!
  ...
  call ss_lg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, t2, &
...
!
    call ss_lg_rpl_pop ( i1sub, j1sub, j1type, t1, i3sub, j3sub, j3type, t3, &
      stack1, stack2, stack_num, stack_max )
!
...
!
      ...
      i2sub = ( i1sub + i3sub ) / 2
!
!  Note that THESE score calculations are done using the GLOBAL alignment rules.
!
      base = t1
...
!
      ...
      if ( j2type == MATCH ) then
        ...
        call ss_lg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
          t2, stack1, stack2, stack_num, stack_max )

        call ss_lg_rpl_push ( i2sub, j2sub, j2type, t2, i3sub, j3sub, j3type, &
        ...
        if ( ( i1sub < i2sub-1 .or. j1sub < j2sub ) .and. &
          ...
          call ss_lg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub-1, j2sub, &
            j2type, t2, stack1, stack2, stack_num, stack_max )

          call ss_lg_rpl_push ( i2sub+1, j2sub, j2type, t2, i3sub, j3sub, &
...
subroutine ss_lg_rpl_pop ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
  t2, stack1, stack2, stack_num, stack_max )
...
!  Parameters:
!
!    Output, integer I1SUB, J1SUB, J1TYPE, real T1, the row and column of 
!    the first cell, the type of the match there, and the appropriate base.
!
!    Output, integer I2SUB, J2SUB, J2TYPE, real T2, the row and column of 
!    the second cell, the type of the match there, and the appropriate base.
!
...
!
  ...
  if ( stack_num < 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'SS_LG_RPL_POP - Fatal error!'
...
subroutine ss_lg_rpl_push ( i1sub, j1sub, j1type, t1, i2sub, j2sub, j2type, &
  t2, stack1, stack2, stack_num, stack_max )
...
!
  if ( stack_num >= stack_max ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'SS_LG_RPL_PUSH - Fatal error!'

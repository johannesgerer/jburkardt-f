...
!*****************************************************************************80
!
!! MAIN is the main program for BUFFON_MPI.
!
!  Discussion:
!
...
!
  ...
  write ( *, '(2x,i8,2x,i12,2x,g14.6)' ) id, seed, random_value
!
...
!
  ...
  call MPI_Reduce ( hit_num, hit_total, 1, MPI_INTEGER, MPI_SUM, 0, &
    MPI_COMM_WORLD, ierr )
!
...
!
  if ( id == 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'BUFFON_MPI - Master process:'
...
!
!    Output, integer ( kind = 4 ) BUFFON_LAPLACE_SIMULATE, the number of times the needle

...
!  Local parameters:
!
!    Local, integer DIM_NUM, the spatial dimension of the point set.
!
!    Local, integer N, the number of points.
!
!    Local, real ( kind = 8 ) Z(DIM_NUM,N), the point set.
!
...
!
  ...
  integer ( kind = 4 ) n
  integer ( kind = 4 ), parameter :: ns = 100000
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TABLE_QUALITY:'
  write ( *, '(a)' ) '  FORTRAN90 version:'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Compute measures of uniform dispersion for a pointset.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Note: this routine assumes that the input pointset'
  write ( *, '(a)' ) '  is contained in the unit hypercube.  If this is not'
  write ( *, '(a)' ) '  the case, then you must rewrite the routine'
  write ( *, '(a)' ) '    SAMPLE_ROUTINE'
  write ( *, '(a)' ) '  so that it properly returns sample points in your'
  write ( *, '(a)' ) '  region, with a uniform density, or a probability'
  write ( *, '(a)' ) '  density of your choosing.'
...
!
  ...
  value = beta_measure ( dim_num, n, z )
  write ( *, '(a,g16.8)' ) '  Relative spacing deviation    Beta = ', value
  value = chi_measure ( dim_num, n, z, ns, sample_routine, seed_init )
  write ( *, '(a,g16.8)' ) '  The regularity measure         Chi = ', value
  value = d_measure ( dim_num, n, z, ns, sample_routine, seed_init )
  write ( *, '(a,g16.8)' ) '  2nd moment determinant measure   D = ', value
  value = e_measure ( dim_num, n, z, ns, sample_routine, seed_init )
  write ( *, '(a,g16.8)' ) '  The Voronoi energy measure       E = ', value
  value = gamma_measure ( dim_num, n, z )
  write ( *, '(a,g16.8)' ) '  The mesh ratio               Gamma = ', value
  value = h_measure ( dim_num, n, z, ns, sample_routine, seed_init )
  write ( *, '(a,g16.8)' ) '  The point distribution norm      H = ', value
  value = lambda_measure ( dim_num, n, z )
  write ( *, '(a,g16.8)' ) '  The COV measure             Lambda = ', value
  value = mu_measure ( dim_num, n, z, ns, sample_routine, seed_init )
  write ( *, '(a,g16.8)' ) '  The point distribution ratio    Mu = ', value
  value = nu_measure ( dim_num, n, z, ns, sample_routine, seed_init )
  write ( *, '(a,g16.8)' ) '  The cell volume deviation       Nu = ', value
  value = r0_measure ( dim_num, n, z )
  write ( *, '(a,g16.8)' ) '  The Riesz 0-level energy        R0 = ', value
!
!  The S measure is only appropriate if the pointset is in the
!  unit hypercube.
!
  if ( r8mat_in_01 ( dim_num, n, z ) ) then
    value = sphere_measure ( dim_num, n, z )
    write ( *, '(a,g16.8)' ) '  Nonintersecting sphere volume    S = ', value
  else
    write ( *, '(a,g16.8)' ) '  Nonintersecting sphere volume    S = (omitted)'
  end if

  value = tau_measure ( dim_num, n, z, ns, sample_routine, seed_init )
  write ( *, '(a,g16.8)' ) '  2nd moment trace measure       Tau = ', value
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a,g16.8)' ) '  Minimum spacing          Gamma_min = ', gamma_min
  write ( *, '(a,g16.8)' ) '  Average spacing          Gamma_ave = ', gamma_ave
  write ( *, '(a,g16.8)' ) '  Maximum spacing          Gamma_max = ', gamma_max
  write ( *, '(a,g16.8)' ) '  Spacing standard deviate Gamma_std = ', gamma_std
!
!  Free memory.
!
  deallocate ( gamma )
...
!  Discussion:
!
!    This version of the routine simply calls the built in routine
!    for sampling the unit hypercube with uniform density.  
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!
  ...
  call sample_hypercube_uniform ( dim_num, n, seed, x )
...
!  Reference:
!
!    Douglas Hardin and Edward Saff,
...
!    triangle neighbor list; negative values are used for links of a counter 
!    clockwise linked list of boundary edges;
!      LINK = -(3*I + J-1) where I, J = triangle, edge index.
!

...
!****************************************************************************80
!
!! MAIN is the main program for MXM2.
!
!  Discussion:
!
!    This example is the same as the MXM example, except that now we try
!    to measure the CPU time required by each thread.
!
!  Licensing:
!
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The number of processors available = ', omp_get_num_procs ( )
  write ( *, '(a)' ) '  The number of threads available    = ', omp_get_max_threads ( )
...
!
  ...
  real    ( kind = 8 ) cpu1
  real    ( kind = 8 ) cpu2
  real    ( kind = 8 ), allocatable :: cpu_times (:)
  integer ( kind = 4 ) i
  integer ( kind = 4 ) id
  integer ( kind = 4 ) j
  integer ( kind = 4 ) k
...
!
!  Create array for separate CPU times.
!
  p = omp_get_max_threads ( )

  allocate ( cpu_times(0:p-1) )

  cpu_times(0:p-1) = 0.0D+00
!
!  Compute A = B * C.
!
...
!$omp parallel &
!$omp shared ( a, b, c, l, m, n, cpu_times ) &
!$omp private ( cpu1, cpu2, i, id, j, k )
...
!$omp do
  do j = 1, n
    ...
    call cpu_time ( cpu2 )
    cpu_times(id) = cpu_times(id) + cpu2 - cpu1
  end do
!$omp end do
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  CPU timings for individual threads:'
  write ( *, '(a)' ) ' '
  do id = 0, p - 1
    write ( *, '(2x,i8,2x,g14.6)' ) id, cpu_times(id)
  end do

...
!
  ...
  integer ( kind = 4 ), parameter :: tetra_order2 = 10
...
!  Read the tetra data.
!
  call i4mat_header_read ( input_element_filename, tetra_order1, tetra_num )

  if ( tetra_order1 /= 4 ) then
    write ( *, * ) ' '
    write ( *, '(a)' ) 'TET_MESH_L2Q - Fatal error!'
  ...
  allocate ( tetra_node1(1:tetra_order1,1:tetra_num) )

  call i4mat_data_read ( input_element_filename, tetra_order1, tetra_num, &
    tetra_node1 )
  ...
  if ( verbose ) then
    ...
    call i4mat_transpose_print_some ( tetra_order1, tetra_num, tetra_node1, &
      1, 1, tetra_order1, 5, '  First 5 tetrahedrons:' )
  end if
!
!  Detect and correct 0-based indexing.
!
  call mesh_base_one ( node_num1, tetra_order1, tetra_num, tetra_node1 )
!
!  Compute the quadratic mesh.
!
  allocate ( edge_data(1:5,1:6*tetra_num) )

  call tet_mesh_order4_to_order10_size ( tetra_num, tetra_node1, node_num1, &
    edge_data, node_num2 )

  write ( *, '(a,i8)' ) '  Number of quadratic nodes = ', node_num2

  allocate ( node_xyz2(1:dim_num,1:node_num2) )
  allocate ( tetra_node2(1:tetra_order2,1:tetra_num) )

  call tet_mesh_order4_to_order10_compute ( tetra_num, tetra_node1, node_num1, &
    node_xyz1, edge_data, tetra_node2, node_num2, node_xyz2 )
!
...
!
  ...
  call i4mat_transpose_print_some ( tetra_order2, tetra_num, tetra_node2, &
...
!
  ...
  call i4mat_write ( output_element_filename, tetra_order2, tetra_num, &
    tetra_node2 )

  write ( *, '(a)' ) '  Wrote the quadratic tetrahedrons to "' &
...
!    linear mesh.
!
!    Input, integer ( kind = 4 ) TETRA_NODE1(4,TETRA_NUM), the indices of the nodes
!    in the linear mesh.
!
!    Input, integer ( kind = 4 ) NODE_NUM1, the number of nodes for the linear mesh.
!
...
!
!    Output, integer ( kind = 4 ) TETRA_NODE2(10,TETRA_NUM), the indices of the nodes
!    in the quadratic mesh.
!
!    Input, integer ( kind = 4 ) NODE_NUM2, the number of nodes for the quadratic mesh.
!
...
!
!    Output, integer ( kind = 4 ) NODE_NUM2, the number of nodes for the quadratic mesh.
!
  implicit none

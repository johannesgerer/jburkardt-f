#!/bin/bash
#
gfortran -c -g sgmga_product_weight_prb.f90 >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling sgmga_product_weight_prb.f90"
  exit
fi
rm compiler.txt
#
gfortran sgmga_product_weight_prb.o -L$HOME/lib/$ARCH -lsgmga -lsandia_rules
if [ $? -ne 0 ]; then

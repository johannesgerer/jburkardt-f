...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the polynomial exactness of a generalized Gauss-Laguerre'
  write ( *, '(a)' ) '  quadrature rule by integrating exponentially weighted '
  write ( *, '(a)' ) '  monomials up to a given degree over the [0,+oo) interval.'
  write ( *, '(a)' ) ' '
...
!
  if ( 4 <= arg_num ) then
    ...
    write ( *, '(a)' ) '  0: standard generalized rule for integrating x^alpha*exp(-x)*f(x);'
...
!
  ...
  if ( option == 0 ) then
    ...
    write ( *, '(a)'       ) '    Integral ( A <= x < +oo ) x^alpha exp(-x) f(x) dx'
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  A generalized Gauss-Laguerre rule would be able to exactly'
  write ( *, '(a,i8)' ) &

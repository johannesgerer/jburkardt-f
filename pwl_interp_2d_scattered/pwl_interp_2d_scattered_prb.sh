#!/bin/bash
#
gfortran -c -g pwl_interp_2d_scattered_prb.f90 >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling pwl_interp_2d_scattered_prb.f90"
  exit
fi
rm compiler.txt
#
gfortran pwl_interp_2d_scattered_prb.o -L$HOME/lib/$ARCH -lpwl_interp_2d_scattered -ltest_interp_2d -lr8lib
if [ $? -ne 0 ]; then
